...
填一次校园网账号密码，之后连上校园网即可自动认证；掉线自动重连，可常驻后台运行，关掉窗口也不影响联网。

![主界面截图](docs/screenshots/01-connect.png)

## 主要特性

- **一键认证**：自动检测网络状态 → 提交锐捷 ePortal / CAS 登录。
- **后台保活**：关掉窗口后仍在后台持续检测与重连，不占用任务栏位置。
- **低资源占用**：在线稳定后自动放宽检测间隔，并降低进程优先级与 QoS。
- **多设备互斥**：账号通常只允许一台设备在线。程序只读地查询自助服务的在线设备 MAC，确认是其它设备在用网时主动礼让，避免电脑与手机互相踢下线。
- **换学校可用**：认证网关可手动填写，也可点「自动探测网关」自动发现。
- **自动检查更新**：内置 GitHub Releases 版本检查，已指向本仓库。
...
需要 Python 3.13+（已测试 3.13.12），建议安装 Pillow（缺失时图标会降级为整数降采样）。
...
会在项目根目录生成 `CampusLogin.exe`，并复制到干净的对外发布目录。打包脚本会自动在打包前后停止 / 启动后台保活。
